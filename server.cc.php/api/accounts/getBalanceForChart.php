...
for ($i = 28; $i > -1; $i--) {
    $sub = mysqli_query($connectDB, "select sum(amountMoney) from users_transaction 
            where (dateTransaction between SUBDATE(curdate(), interval $i day) and ADDDATE(curdate(), interval 0 day)) 
                and (isOutcome=true or isIncome=true) and userID=$userid");
    $sub = mysqli_fetch_array($sub);
    $sub = $sub[0];
    array_push($arr, $allmoney - $sub);
}
