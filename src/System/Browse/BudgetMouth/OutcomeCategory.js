      ...
      <td>
        <div className="progress">
          ...
          <span className="balance-completed">
            <span className="amount">{Math.abs(props.outcomeMoney)}&nbsp;руб.</span>
          </span>
        </div>
      </td>
    </tr>
...
const OutcomeCategory = (props) => {
  const outcomeCategoryElement = props.reportCategories.map(each => <OutcomeCategoryItem {...each} />)
  ...
  return (
    <div className="col-8" >
      <table className="table">
        ...
        <tbody>
          {props.countReportCategories===0 ? <tr><td><h4>В этом месяце нет трат</h4></td></tr> : outcomeCategoryElement}
        </tbody>
      </table>
    </div>
  )
