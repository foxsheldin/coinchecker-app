    ...
    case SET_CATEGORIES:
      switch (action.typeCategories) {
        case "income":
          return {
            ...state,
            categories: { ...state.categories, income: action.categories },
          };
        case "outcome":
          return {
            ...state,
            categories: { ...state.categories, outcome: action.categories },
          };
        default:
          return state;
      }
...
const setEditTransaction = (editTransaction) => ({
  type: SET_EDIT_TRANSACTION,
  editTransaction,
});
const setCategories = (categories, typeCategories) => ({
  type: SET_CATEGORIES,
  categories,
  typeCategories,
});
const setTotalTransactionsCount = (totalCount) => ({
  type: SET_TOTAL_COUNT,
  totalCount,
});
const setReportCategories = (reportCategories, countReportCategories) => ({
  type: SET_REPORT_CATEGORIES,
  reportCategories,
  countReportCategories,
});
const setDataCashFlow = (dataCashFlow) => ({
  type: SET_DATA_CASH_FLOW,
  dataCashFlow,
});
...
export const getTransactions = (
  ...
  return (dispatch) => {
    ...
    transactionsAPI
      .getTransactions(currentPage, pageSize, userid, typeAccount, accountID)
      .then((data) => {
...
export const deleteTransaction = (transactionid, userid) => {
  return async (dispatch, getState) => {
    let response = await transactionsAPI.deleteTransaction(
      transactionid,
      userid
    );
...
export const getEditTransaction = (transactionid, userid) => {
  return (dispatch) => {
    ...
    transactionsAPI.getEditTransaction(transactionid, userid).then((data) => {
      data.items = {
        ...data.items,
        amountMoney: Math.abs(data.items.amountMoney),
      };
...
export const getReportCategories = (userid) => {
  return async (dispatch) => {
    ...
    if (response.resultCode === 0) {
      dispatch(
        setReportCategories(
          response.reportCategories,
          response.countReportCategories
        )
      );
    }
    dispatch(toggleIsFetching(false));
...
export const getCashFlowForDoughnut = (userid) => {
  return async (dispatch) => {
    ...
    if (response.resultCode === 0) {
      dispatch(
        setDataCashFlow({ income: response.income, outcome: response.outcome })
      );
    }
    dispatch(toggleIsFetching(false));
  };
